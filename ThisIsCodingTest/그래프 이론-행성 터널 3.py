...
distances = []
for i in range(N-1):
    heapq.heappush(distances,(planetsX[i+1][0]-planetsX[i][0],planetsX[i+1][1],planetsX[i][1]))
    heapq.heappush(distances,(planetsY[i+1][0]-planetsY[i][0],planetsY[i+1][1],planetsY[i][1]))
    heapq.heappush(distances,(planetsZ[i+1][0]-planetsZ[i][0],planetsZ[i+1][1],planetsZ[i][1]))
parent = list(range(N))
print(distances)
# distances.sort()
result = 0
