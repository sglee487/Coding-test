def solution(numbers):
    print(numbers)
    numstrs = [str(n) for n in numbers]
    print(sorted(numstrs))
    maxdigitlen = max(map(lambda x:len(x),numstrs))
    numstrdigit = sorted([(s.ljust(maxdigitlen,s[-1]), int(len(s))) for s in numstrs],key=lambda x:(-int(x[0]),x[1]))
    answer = ""
    for ns, nd in numstrdigit:
        answer += ns[:nd]
    print(numstrdigit, answer)
...
print(solution([0,0,0,1000]), "1000")
